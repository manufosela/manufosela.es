export const leadershipData = {
  es: {
    heroBadge: 'Lanzamiento Exclusivo',
    heroTitle: 'Liderazgo',
    heroTitleAccent: 'Afectivo',
    heroDescription: 'Aprende a liderar con el corazón y la mente. Una guía para la nueva era de cultura organizacional.',
    buyButton: 'Comprar Libro',
    detailsButton: 'Más sobre el libro',
    buyLink: 'https://savvily.es/libros/liderazgo-afectivo/',
    detailsLink: 'https://liderazgoafectivo.com',
    editorialNote: 'Publicado por Savvily Editorial',
    bookCover: '/images/portada_libro_es.jpg',
    bookAlt: 'Portada del libro Liderazgo Afectivo de Mánu Fosela — Editorial Savvily',
    manifestoLabel: 'People & Culture Manifesto',
    manifesto: 'El verdadero liderazgo no reside en la jerarquía, sino en la conexión humana. Las organizaciones no son estructuras; son ecosistemas de confianza y afecto técnico.',
    manifestoAccent: 'conexión humana',
    communityTitle: 'Comunidad',
    communityDescription: 'Únete al movimiento de líderes conscientes.',
    communityMembers: '500+',
    communityMembersLabel: 'Miembros',
    communityButton: 'Unirse a la Comunidad',
    communityLink: 'https://comunidad.liderazgoafectivo.com',
    testimonial: 'La mentoría de Mánu transformó no solo mis métricas, sino la forma en que mi equipo se siente cada lunes al llegar a trabajar.',
    testimonialAuthor: 'Javier Aranda',
    testimonialRole: 'CTO, TechFlow Solutions',
    insightsItems: [
      { icon: 'terminal', title: 'Tech Leadership', description: 'Frameworks técnicos aplicados a la gestión de personas de alto rendimiento.' },
      { icon: 'chart', title: 'Métricas', description: 'KPIs de bienestar y cultura organizacional.' },
      { icon: 'mail', title: 'Contacto', description: 'Mentorías personalizadas para líderes.' }
    ]
  },
  en: {
    heroBadge: 'Exclusive Launch',
    heroTitle: 'Affective',
    heroTitleAccent: 'Leadership',
    heroDescription: 'Learn to lead with heart and mind. A guide for the new era of organizational culture.',
    buyButton: 'Buy Book',
    detailsButton: 'More about the book',
    buyLink: 'https://www.amazon.com/dp/B0F1Y8XG2G',
    detailsLink: 'https://liderazgoafectivo.com',
    editorialNote: 'Published by Savvily Editorial',
    bookCover: '/images/portada_libro_en.png',
    bookAlt: 'Book cover of Affective Leadership by Mánu Fosela — Savvily Editorial',
    manifestoLabel: 'People & Culture Manifesto',
    manifesto: 'True leadership does not reside in hierarchy, but in human connection. Organizations are not structures; they are ecosystems of trust and technical affection.',
    manifestoAccent: 'human connection',
    communityTitle: 'Community',
    communityDescription: 'Join the movement of conscious leaders.',
    communityMembers: '500+',
    communityMembersLabel: 'Members',
    communityButton: 'Join the Community',
    communityLink: 'https://comunidad.liderazgoafectivo.com',
    testimonial: 'Mánu\'s mentorship transformed not just my metrics, but the way my team feels every Monday when they come to work.',
    testimonialAuthor: 'Javier Aranda',
    testimonialRole: 'CTO, TechFlow Solutions',
    insightsItems: [
      { icon: 'terminal', title: 'Tech Leadership', description: 'Technical frameworks applied to high-performance people management.' },
      { icon: 'chart', title: 'Metrics', description: 'Well-being and organizational culture KPIs.' },
      { icon: 'mail', title: 'Contact', description: 'Personalized mentorship for leaders.' }
    ]
  }
};
